      ...
      if (_rng.nextDouble() < 0.3) { // 30% chance this intersection is affected
        switch (aiAlert.type) {
          ...
          case AIAlertType.emergencyVehicleDetected:
            // Override signal for emergency vehicle
            intersection.manualOverride = true;
            intersection.phase = SignalPhase.green;
            intersection.forcedPhase = SignalPhase.green;
            intersection.lastAction = 'Emergency vehicle detected - signal overridden';
            intersection.lastActionAt = DateTime.now();
            break;
          case AIAlertType.emergencyVehicleApproaching:
            // Prepare for emergency vehicle
            intersection.remainingSeconds = 90; // Extend current phase
            intersection.lastAction = 'Preparing for approaching emergency vehicle';
            intersection.lastActionAt = DateTime.now();
            break;
        }
      }
    ...
    switch (sensorData.sensorType) {
      ...
      case IoTSensorType.emergencyVehicleDetector:
        if (sensorData.value == 1.0) {
          ...
          relevantIntersection.manualOverride = true;
          relevantIntersection.phase = SignalPhase.green;
          relevantIntersection.forcedPhase = SignalPhase.green;
          relevantIntersection.lastAction = 'Emergency vehicle detected - overriding signal';
          relevantIntersection.lastActionAt = DateTime.now();
        }
        break;
      case IoTSensorType.audioSensorSiren:
        if (sensorData.value > 95.0) {
          // Siren detected - prepare for emergency vehicle
          relevantIntersection.remainingSeconds = 90; // Extend current phase
          relevantIntersection.lastAction = 'Emergency siren detected - extending phase';
          relevantIntersection.lastActionAt = DateTime.now();
        }
        break;
    }
